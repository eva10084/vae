...
TestDir = dataset_dir+'/LGE_Vali/'  # 代表验证集数据所在的路径，mri测试集
...
# 对测试集mri进行分割
def SegNet_test_mr(dir, mrSegNet, gate,epoch,ePOCH, save_DIR): # gate=0
    criterion = 0
    ...
    num = 0
    mrSegNet.eval()   # 设定网络为评估模式，这意味着网络不进行反向传播和梯度更新操作。

    # 读取文件夹中的某个病例的MRI图像和对应的分割标签，将标签转换为分类标签并将图像和标签分别保存在npimg和nplab变量中
    for i in range(len(labsname)):
        itklab = sitk.ReadImage(labsname[i])   # 读取第i个病例的分割标签。
        ...
        data = torch.from_numpy(np.expand_dims(npimg, axis=1)).type(dtype=torch.FloatTensor)
        # label = torch.from_numpy(nplab).cuda()
        label = torch.from_numpy(nplab)
...
def main():
    ...
    # 使用DA模型中的编码器vaeencoder对训练集和测试集进行编码，然后进行t-SNE可视化
